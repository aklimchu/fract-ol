...
/*   Updated: 2024/07/16 14:34:58 by aklimchu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fractol.h"

static void	fractal_man(t_data *img, t_calc calc);

static void	fractal_man_tools(t_data *img, t_calc calc);
...
static void	fractal_man(t_data *img, t_calc calc)
{
	...
	while (calc.y <= SCREEN_H - 2)
	{
		...
		{
			calc.c.real = calc.x * calc.scale.real + calc.dim.left_f;
			calc.c.imag = calc.y * calc.scale.imag + calc.dim.top_f;
			calc.z.real = 0;
			calc.z.imag = 0;
			fractal_man_tools(img, calc);
			calc.x++;
		}
		calc.y++;
	}
