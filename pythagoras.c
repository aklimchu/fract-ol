...
static void	fractal_pyth(t_vars *vars, t_pyth a, t_pyth b, int times)
{
	...
	vars->img.color = gradient(vars->startcolor, vars->endcolor, \
		MAXTIMES + 1, times);
	c.x = b.x - (a.y - b.y);
	c.y = b.y - (b.x - a.x);
	d.x = a.x - (a.y - b.y);
	...
	if (times > 0 && check_borders(vars, c, d, e) == 0)
	{
...
static int	check_borders(t_vars *vars, t_pyth c, t_pyth d, t_pyth e)
{
	if (vars->a.x >= 0 && vars->a.y >= 0 && vars->b.x >= 0 && \
		vars->b.y >= 0 && c.x >= 0 && c.y >= 0 && d.x >= 0 && \
		d.y >= 0 && e.x >= 0 && e.y >= 0 && vars->a.x < SCREEN_W && \
		vars->a.y < SCREEN_H && vars->b.x < SCREEN_W && \
		vars->b.y < SCREEN_H && c.x < SCREEN_W && c.y < SCREEN_H && \
		d.x < SCREEN_W && d.y < SCREEN_H && e.x < SCREEN_W && e.y < SCREEN_H)
		return (0);
	return (1);
}
